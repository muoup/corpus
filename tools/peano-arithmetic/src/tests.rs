...
use corpus_core::expression::{LogicalExpression, DomainContent};
use crate::syntax::{PeanoContent, PeanoExpression};
...
fn test_domain_content() {
    ...
    let equals_content = PeanoContent::Equals(
        crate::syntax::ArithmeticExpression::Term(
            crate::syntax::Term::Number(0.into())
        ).into(),
        crate::syntax::ArithmeticExpression::Term(
            crate::syntax::Term::Number(0.into())
        ).into()
    ...
    assert_eq!(result, None, "Equals should return None for evaluation without context");
